            ...
            <div className="BorrowBookTicketWrapper">
                ...
                <div style={{ display: "flex", gap: "20rem", margin: "2rem", marginLeft: "5rem", marginRight: "5rem" }}>
                    <div>
                        ...
                        <p>Địa chỉ: {userData.contactInfo || "Không có thông tin"}</p>
                        <p>Ngày mượn: {new Date().toLocaleDateString()}</p>
                    </div>
                    <div>
                        <p>Id bạn đọc: {readerId}</p> {/* ID bạn đọc lấy từ localStorage */}
                        <p>Số điện thoại: {userData.contactInfo || "Không có thông tin"}</p>
